...
import { type ListSort, type RowSortFieldMap, getItem, getItemRows, getList } from '@/models/accounts.ts'
...
export const item: ItemHandler<AnyObject, AnyObject, RowsQueryString<RowSortFieldMap>> = async ({ query, params }) => {
  ...
  const { data, accountId, firstTx, lastTx } = itemEntry instanceof Promise ? await itemEntry : itemEntry

  const rowsEntry = query.rows
    ? await getEntry(
        `${cacheKey}Rows|${accountId}|${query.rows}|${query.sort}|${dir}|${limit}|${after}|${policy}`,
        () =>
          getItemRows({
            rows: query.rows,
            sort: query.sort,
