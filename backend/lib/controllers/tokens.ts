...
import { type ListSort, type RowSortFieldMap, getItem, getItemRows, getList } from '@/models/tokens.ts'
...
export const item: ItemHandler<AnyObject, AnyObject, RowsQueryString<RowSortFieldMap>> = async ({ query, params }) => {
  ...
  const { data, tokenId } = itemEntry instanceof Promise ? await itemEntry : itemEntry

  const rowsEntry = query.rows
    ? await getEntry(`${cacheKey}Rows|${tokenId}|${query.rows}|${query.sort}|${dir}|${limit}|${after}`, () =>
        getItemRows({
          rows: query.rows,
          sort: query.sort,
          dir,
